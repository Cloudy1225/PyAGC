...
def log_system_info(model, data, device, logger, conf):
    ...
    logger.info(f"SVD iterations (tau): {model.tau}")
    logger.info(f"Oversampling parameter: {model.oversampling}")
...
def main():
    parser = argparse.ArgumentParser(description='S2CAG for node clustering')
    ...
    model = S2CAG(
        ...
        tau=conf.get('tau', 7),
        oversampling=conf.get('oversampling', 10)
    )
    ...
    # Note: S2CAG has no training phase, it directly computes embeddings
    logger.info("=" * 60)
    logger.info("S2CAG is a training-free method")
