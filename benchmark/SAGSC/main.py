...
from pyagc.models import SAGSC
...
def log_system_info(model, data, device, logger, conf):
    ...
    logger.info(f"Propagation order (p): {model.p}")
    logger.info(f"Number of clusters (k): {model.k}")
    logger.info(f"Quadratic kernel bias: {model.bias}")
...
def main():
    parser = argparse.ArgumentParser(description='SAGSC for node clustering')
    ...
    # Create SAGSC model
    model = SAGSC(
        p=conf.get('num_layers', 2),
        k=n_clusters,
        bias=conf.get('bias', 2 ** -0.5)
    )
    ...
    # Note: SAGSC has no training phase, it directly computes embeddings
    logger.info("=" * 60)
    logger.info("SAGSC is a training-free method")
