...
from pyagc.models import DinkNet
...
def main():
    parser = argparse.ArgumentParser(description='DinkNet for node clustering')
    ...
    # Create projector
    projector = MLP(
        in_channels=conf.get('hidden_channels', 512),
        hidden_channels=conf.get('hidden_channels', 512),
        out_channels=conf.get('hidden_channels', 512),
        num_layers=conf.get('projector_layers', 1)
    )
