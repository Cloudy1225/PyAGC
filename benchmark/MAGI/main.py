...
from pyagc.models import MAGI
from pyagc.models.magi import MAGINeighborLoader, precompute_full_graph_positive_pairs
...
def train_mini_batch(model, data, optimizer, conf, logger, device,
    ...
    logger.info("=" * 60)
    logger.info("Training Mode: Mini-batch (MAGINeighborLoader)")
    logger.info("=" * 60)
    ...
    num_walks = conf.get('num_walks', 20)
    walk_length = conf.get('walk_length', 4)

    logger.info(f"Batch size: {batch_size}")
    logger.info(f"Fan-out: [{fan_out}] × {num_layers} layers")
    logger.info(f"Random walks: {num_walks} walks × {walk_length} steps")
...
def main():
    parser = argparse.ArgumentParser(description='MAGI for node clustering')
    ...
    model = MAGI(
        encoder=encoder,
        tau=conf.get('tau', 0.5),
        scale_embeddings=conf.get('scale_embeddings', True),
    ).to(device)
    ...
    if skip_training:
        ...
        # Precompute positive pairs for full-batch training
        if not mini_batch_training:
            logger.info("=" * 60)
            logger.info("Preprocessing: Computing positive pairs for full-graph training")
            logger.info("=" * 60)
            precompute_start = time.time()
            data.pos_pairs = precompute_full_graph_positive_pairs(
                data,
                num_walks=conf.get('num_walks', 100),
                walk_length=conf.get('walk_length', 2),
            )
            precompute_time = time.time() - precompute_start
            logger.info(f"✓ Positive pairs computed in {precompute_time:.2f}s")
            logger.info(f"  Number of positive pairs: {data.pos_pairs.shape[1]:,}")
