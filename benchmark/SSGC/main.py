...
def inference_embeddings(model, data, conf, logger, device, labeled_indices=None):
    ...
    Args:
        model: The model (no parameters, so device doesn't matter much)
...
def log_system_info(model, data, device, logger, conf):
    ...
    logger.info(f"Number of hops (K): {model.K}")
    logger.info(f"Teleport probability (alpha): {model.alpha}")
    logger.info(f"Cached: {model.cached}")
    logger.info(f"Add self loops: {model.add_self_loops}")
...
def main():
    parser = argparse.ArgumentParser(description='SSGC for node clustering')
    ...
    # Create SSGC model
    model = SSGC(
        alpha=conf.get('alpha', 0.05),
        K=conf.get('num_layers', 2),
        cached=conf.get('cached', False),
        add_self_loops=conf.get('add_self_loops', True)
    )
    ...
    # Note: SSGC is a training-free method, it directly computes embeddings
    logger.info("=" * 60)
    logger.info("SSGC is a training-free method")
